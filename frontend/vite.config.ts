...
const resolveConfig: UserConfig["resolve"] = {
  alias: [
...
const optimizeDependencies: NonNullable<UserConfig["optimizeDeps"]> = {
  include: [
    ...
    "react-is", "react-router-dom", "recharts", "sonner", "tailwind-merge",
  ],
};

function createServerConfig(overlayLoaded: boolean): NonNullable<UserConfig["server"]> {
  return {
    host: true,
    port: 3000,
    allowedHosts: true,
    ...
    cors: true,
    // No hmr.clientPort override: Vite infers the WS target from window.location.
    hmr: hotReloadDisabled ? false : { overlay: !overlayLoaded },
    watch: hotReloadDisabled ? null : { usePolling: true, interval: 300 },
    // Frontend code calls relative /api/* through the FastAPI dev-server proxy.
...
async function createViteConfig(): Promise<UserConfig> {
  ...
  return {
    plugins: [react(), tailwindcss(), ...(visualEditsDisabled ? [] : [visualEdits()]), ...(emergentOverlay ? [emergentOverlay] : [])],
    resolve: resolveConfig,
    optimizeDeps: optimizeDependencies,
    server: createServerConfig(Boolean(emergentOverlay)),
  };
}
